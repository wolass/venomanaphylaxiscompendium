...
```{r adrenuse, fig.cap="**Therapy of IVA cases.** A: General managment methods in IVA and non-IVA. B: Reasons for not administering adrenaline in both IVA and non-IVA cases. C,D,E: Age, sex and severity distribution was matched in cases in both groups to allow for comparable results between IVA and non_IVA cases"}
fig_adrenuse1
```
...
```{r flow,fig.width=6,fig.height=6}
ggarrange(ggplot()+
...
```{R adrenalineuse, fig.cap = "The proportion of insect venom elicited anaphylaxis cases that were treated with adrenaline in comparison to other elicitors.",fig.height=4,fig.width = 6}
fig_adrenuse1
```

##### pagebreak

```{r figsymptoms, fig.cap="Symptoms of insect venom anaphylaxis (IVA) compared to other elicitors",fig.width=9.5,fig.height = 6}
fig_symptoms
```
...
```{r figcofactors, fig.cap="Cofactors of insect venom anaphylaxis ad their relationship to severity of a reaction",fig.width=4.2,fig.height = 6}
cof_fig
```
